...
func startGRPCServer(address, certFile, keyFile string) (*grpc.Server, error) {
	...
	userspb.RegisterUsersServiceServer(grpcServer, &s)
...
func startRESTServer(address, grpcAddress, certFile string) (*http.Server, error) {
	...
	err = userspb.RegisterUsersServiceHandlerFromEndpoint(ctx, mux, grpcAddress, opts)
...
func main() {
	grpcAddress := fmt.Sprintf("%s:%d", "localhost", 8888)
	restAddress := fmt.Sprintf("%s:%d", "localhost", 8889)
